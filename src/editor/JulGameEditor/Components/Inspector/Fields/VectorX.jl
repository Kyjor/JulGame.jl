function show_field(structure::EditableStructure, field::Symbol, value::Union{Math._Vector2{Float64}, Math._Vector2{Int32}})
    ...
    if is_float
        @c CImGui.DragFloat2("##$(string(field))", val, 0.1, -max_value, max_value, "%.3f", slider_flags)
    else
        @c CImGui.DragInt2("##$(string(field))", val, 1, -max_value, max_value, "%d", slider_flags)
    end
    CImGui.PopItemWidth()
...
function show_field(structure::EditableStructure, field::Symbol, value::Union{Math._Vector4{Float64}, Math._Vector4{Int32}})
    ...
    CImGui.PushItemWidth(-1)  # fill remaining width
    if is_float
        @c CImGui.DragFloat4("##$(string(field))", val, 0.1, -max_value, max_value, "%.3f", slider_flags)
    else
        @c CImGui.DragInt4("##$(string(field))", val, 1, -max_value, max_value, "%d", slider_flags)
    end
    CImGui.PopItemWidth()
