...
function show_effect_panel_for(eff::Any, tb::JulGame.UI.TextBoxModule.TextBox)
    ...
    elseif eff isa EM.DropShadowEffect
        _fx_input_float!("Distance##ds_d", () -> eff.distance, v -> eff.distance = v, 0.5f0, 2.0f0, "%.2f", tb)
        _fx_input_float!("Angle##ds_a", () -> eff.angle, v -> eff.angle = v, 1.0f0, 5.0f0, "%.1f", tb)
        _fx_drag_int!("Blur radius##ds_br", () -> eff.blur_radius, v -> eff.blur_radius = v, 0, 64, tb)
        _fx_color4!("Shadow color", () -> eff.color, v -> eff.color = v, "##ds_col", tb)
        _fx_drag_int!("Opacity##ds_op", () -> eff.opacity, v -> eff.opacity = v, 0, 255, tb)
    elseif eff isa EM.OuterGlowEffect
        _fx_drag_int!("Radius##og_r", () -> eff.radius, v -> eff.radius = v, 0, 128, tb)
        _fx_color4!("Color", () -> eff.color, v -> eff.color = v, "##og_c", tb)
        _fx_input_float!("Blur##og_b", () -> eff.blur, v -> eff.blur = v, 0.05f0, 0.2f0, "%.2f", tb)
        _fx_checkbox!("Force white##og_fw", () -> eff.force_white, v -> eff.force_white = v, tb)
        _fx_input_float!("Fade amount##og_fa", () -> eff.fade_amount, v -> eff.fade_amount = v, 0.05f0, 0.2f0, "%.2f", tb)
        _fx_input_float!("Fade curve##og_fc", () -> eff.fade_curve, v -> eff.fade_curve = v, 0.05f0, 0.2f0, "%.2f", tb)
    elseif eff isa EM.InnerGlowEffect
        _fx_drag_int!("Radius##ig_r", () -> eff.radius, v -> eff.radius = v, 0, 128, tb)
        _fx_color4!("Color", () -> eff.color, v -> eff.color = v, "##ig_c", tb)
    elseif eff isa EM.StrokeEffect
        _fx_drag_int!("Width##st_w", () -> eff.width, v -> eff.width = v, 0, 32, tb)
        _fx_color4!("Color", () -> eff.color, v -> eff.color = v, "##st_c", tb)
    elseif eff isa EM.RoughEdgeEffect
        _fx_drag_int!("Amount##re_a", () -> eff.amount, v -> eff.amount = v, 1, 20, tb)
        _fx_drag_int!("Seed##re_s", () -> eff.seed, v -> eff.seed = v, 0, 2_000_000_000, tb)
        _fx_checkbox!("Erosion##re_e", () -> eff.erosion, v -> eff.erosion = v, tb)
    elseif eff isa EM.InvertEffect
        _fx_checkbox!("Invert red##iv_r", () -> eff.invert_red, v -> eff.invert_red = v, tb)
        _fx_checkbox!("Invert green##iv_g", () -> eff.invert_green, v -> eff.invert_green = v, tb)
        _fx_checkbox!("Invert blue##iv_b", () -> eff.invert_blue, v -> eff.invert_blue = v, tb)
        _fx_checkbox!("Invert alpha##iv_a", () -> eff.invert_alpha, v -> eff.invert_alpha = v, tb)
    elseif eff isa EM.BevelEffect
        _fx_drag_int!("Depth##bv_d", () -> eff.depth, v -> eff.depth = v, 1, 32, tb)
        _fx_input_float!("Angle##bv_a", () -> eff.angle, v -> eff.angle = v, 1.0f0, 5.0f0, "%.1f", tb)
        _fx_slider_float!("Intensity##bv_i", () -> eff.intensity, v -> eff.intensity = v, 0.0f0, 1.0f0, "%.2f", tb)
        _fx_color4!("Highlight", () -> eff.highlight_color, v -> eff.highlight_color = v, "##bv_h", tb)
        _fx_color4!("Shadow", () -> eff.shadow_color, v -> eff.shadow_color = v, "##bv_s", tb)
    elseif eff isa EM.TextureFillEffect
        ...
        _fx_checkbox!("Tile##tf_t", () -> eff.tile, v -> eff.tile = v, tb)
        _fx_combo_int!(
            "Blend##tf_b",
        ...
        _fx_drag_int!("Opacity##tf_o", () -> eff.opacity, v -> eff.opacity = v, 0, 255, tb)
    elseif eff isa EM.GradientEffect
        ...
        _fx_input_float!("Angle (linear)##gr_a", () -> eff.angle, v -> eff.angle = v, 1.0f0, 5.0f0, "%.1f", tb)
    ...
    elseif eff isa EM.BevelEffect1
        ...
        _fx_slider_float!("Intensity##b1_i", () -> Float64(eff.intensity), v -> eff.intensity = Float32(v), 0.0f0, 1.0f0, "%.2f", tb)
        if CImGui.CollapsingHeader("Gradient endpoints##b1_adv")
            _fx_grad_endpoints!(eff.inner_gradient, "Inner", tb)
            _fx_grad_endpoints!(eff.outer_gradient, "Outer", tb)
            _fx_grad_endpoints!(eff.shadow_gradient, "Shadow", tb)
        end
    else
...
function _fx_grad_endpoints!(grad::Vector{JulGame.EffectsModule.GradientStop}, title::String, tb::JulGame.UI.TextBoxModule.TextBox)
