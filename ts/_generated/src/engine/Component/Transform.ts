    ...
    class Transform {
        ...
        constructor(position: Vector3f | Vector2f = Math.Vector3f(0.0,  0.0,  0.0), scale: Vector3f | Vector2f = Math.Vector3f(1.0, 1.0, 1.0), rotation: Vector3f | Vector2f = Math.Vector3f(0.0, 0.0, 0.0), parent = nothing) {
    ...
    function Component_duplicate(this: Transform,  parent: any) {
        let newTransform = Transform(this.position, this.scale, this.rotation, this.parent)
        return newTransform
    }

    function Component_set_position(this: Transform,  position: Vector3f | Vector2f) {
        this.position = position
    }

    function Component_is_mouse_hovering(this: Transform) {
        let mousePosition = JulGame.InputModule.get_mouse_position_in_world_space()
