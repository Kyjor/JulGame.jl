...
import { clamp, haskey, joinpath, mixVolume, unsafe_string } from "../../../../src/engine/core/juliaHelpers";
    ...
    class InternalSoundSource {
        ...
        constructor(parent: any, path: string, channel: number = -1, volume: number = -1, isMusic: boolean = false, playOnStart: boolean = false) {
            ...
            let fullPath = joinpath((globalThis as any).JulGame.BasePath, "assets", "sounds", path)
            let sound = null
            if (path.length > 0) {
            ...
            if ((sound == null || error.length > 0) && path.length > 0) {
                console.log(fullPath)
                console.log(`Error loading file at ${path}. SDL Error: ${error}`);
                (globalThis as any).JulGameSdl.glue_SDL_ClearError()
            }
