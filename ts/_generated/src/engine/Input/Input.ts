...
//todo: separate mouse, keyboard, gamepad, and window into their own files
    ...
    const _hitTestCandidates = []
    const _inactiveCanvasChildren = new Set()
    ...
    function _build_hit_test_candidates() {
        _inactiveCanvasChildren.clear()
        let uiElements = (globalThis as any).MAIN.scene.uiElements
        for (const ui of uiElements) {
            if ((ui?.children != null) && !ui.isActive) {
                for (const child of ui.children) {
                    _inactiveCanvasChildren.add(child)
                }
            }
        }

        let candidates = _hitTestCandidates
        candidates.length = 0
        for (let i = uiElements.length - 1; i >= 0; i--) {
            let ui = uiElements[i]
            if (ui.isActive && !_inactiveCanvasChildren.has(ui)) {
                candidates.push(ui)
            }
        }
        let nUI = candidates.length
        candidates.sort((a, b) => (b.layer ?? 0) - (a.layer ?? 0))

        let entities = (globalThis as any).MAIN.scene.entities
        ...
        return [candidates, nUI] as const
    }
    ...
    export function poll_input(self: Input) {
        ...
        empty(self.buttonsPressedDown)
        empty(self.mouseButtonsPressedDown)
        empty(self.mouseButtonsReleased)  // Clear the released buttons each frame
        ...
        while (true) {
            ...
            if (evt.type == (globalThis as any).JulGameSdl.glue_SDL_MOUSEMOTION || evt.type == (globalThis as any).JulGameSdl.glue_SDL_MOUSEBUTTONDOWN || evt.type == (globalThis as any).JulGameSdl.glue_SDL_MOUSEBUTTONUP) {
                ...
                if (evt.type == (globalThis as any).JulGameSdl.glue_SDL_MOUSEMOTION) {
                    ...
                    if (lastMotionEvent !== null) {
                        _refresh_logical_mouse(self, lastMotionEvent)
                    }
                }
            }
            ...
            if (evt.type == (globalThis as any).JulGameSdl.glue_SDL_MOUSEMOTION || evt.type == (globalThis as any).JulGameSdl.glue_SDL_MOUSEBUTTONDOWN || evt.type == (globalThis as any).JulGameSdl.glue_SDL_MOUSEBUTTONUP) {
                ...
                if (ui_hit_active) {
                    ...
                    for (const element of elementsOrderedByLayerDescending) {
                        ...
                        // handler earlier in self loop can deactivate a canvas
                    ...
                    for (let i = 1; i <= buf.count; i++) {
                        ...
                        if (!clickedAnElementAlready || element.forceClickCheck) {
                            ...
                            if (shouldHandleEvent) {
                                (globalThis as any).JulGame.UI.handle_event(element, evt, mouseX, mouseY)
                                if (evt.type == (globalThis as any).JulGameSdl.glue_SDL_MOUSEBUTTONDOWN) {
            ...
            if (evt.type == (globalThis as any).JulGameSdl.glue_SDL_KEYDOWN || evt.type == (globalThis as any).JulGameSdl.glue_SDL_KEYUP) {
    ...
    */
    function update_joystick_state(self: Input) {
        self.xDir = 0
        self.yDir = 0
        self.button = 0
        if (self.joystick == null) { return }
        if (self.numAxes > 0) {
            let axisX = (globalThis as any).JulGameSdl.glue_SDL_JoystickGetAxis(self.joystick, 0)
        ...
        if (self.numAxes > 1) {
            let axisY = (globalThis as any).JulGameSdl.glue_SDL_JoystickGetAxis(self.joystick, 1)
            if (axisY < -JOYSTICK_DEAD_ZONE) {
                self.yDir = -1
            } else if (axisY > JOYSTICK_DEAD_ZONE) {
                self.yDir = 1
            }
        }

        if (self.numHats > 0) {
            let hat = (globalThis as any).JulGameSdl.glue_SDL_JoystickGetHat(self.joystick, 0);
            (hat & (globalThis as any).JulGameSdl.glue_SDL_HAT_LEFT) != 0 && (self.xDir = -1);
            (hat & (globalThis as any).JulGameSdl.glue_SDL_HAT_RIGHT) != 0 && (self.xDir = 1);
            (hat & (globalThis as any).JulGameSdl.glue_SDL_HAT_UP) != 0 && (self.yDir = -1);
            (hat & (globalThis as any).JulGameSdl.glue_SDL_HAT_DOWN) != 0 && (self.yDir = 1)
        }

        // Rising-edge digital directions (match keyboard get_button_pressed feel)
        if (self.xDir == -1 && self.prevXDir != -1) {
            self.buttonsPressedDown.push("LEFT")
        } else if (self.xDir == 1 && self.prevXDir != 1) {
            self.buttonsPressedDown.push("RIGHT")
        }
        if (self.yDir == -1 && self.prevYDir != -1) {
            self.buttonsPressedDown.push("UP")
        } else if (self.yDir == 1 && self.prevYDir != 1) {
            self.buttonsPressedDown.push("DOWN")
        }
        self.prevXDir = self.xDir
        self.prevYDir = self.yDir

        let buttonHeld = self.numButtons > 0 && (globalThis as any).JulGameSdl.glue_SDL_JoystickGetButton(self.joystick, 0) != 0
        self.button = buttonHeld ? 1 : 0
        if (buttonHeld && !self.prevGamepadButton) {
            self.buttonsPressedDown.push("GAMEPAD_CONFIRM")
        }
        self.prevGamepadButton = buttonHeld
    }
