...
import { haskey, joinpath, pointer, unsafe_string } from "../../../../src/engine/core/juliaHelpers";
    ...
    function load_and_prepare_scene(self: Scene, main = (globalThis as any).JulGame.MainLoop(); 
        ...
        if (!(globalThis as any).JulGame.IS_EDITOR && !(globalThis as any).JulGame.IS_WEB) {
            ...
            @time "scene load: create window" windowCreated = (globalThis as any).JulGame.WindowManagerModule.create_window(windowName, size, isFullscreen, isResizable)
            if (!windowCreated) {
            ...
            @time "scene load: create renderer" createdRenderer = (globalThis as any).JulGameSdl.glue_SDL_CreateRenderer((globalThis as any).MAIN.windowManager.window, -1, (globalThis as any).JulGameSdl.glue_SDL_RENDERER_ACCELERATED)
            (globalThis as any).JulGame.Renderer = createdRenderer
    ...
    */
    ...
    function add_scripts_to_entities(path: string) {
        ...
        let project_scripts_found = false
        if ((globalThis as any).JulGame.ProjectModule != "") {
            ...
            let scripts_mod = filter(
                x => occursin(r"\.Scripts$", String(x)),
                ccall("jl_module_usings", Any, (Any,), getfield(Main, Symbol("$((globalThis as any).JulGame.ProjectModule)")))
            )
        ...
        if (!(globalThis as any).JulGame.IS_PACKAGE_COMPILED && !project_scripts_found) {
            ...
            foreach(file => try
                if (!(file in (globalThis as any).JulGame.LoadedScripts)) {
                    ...
                    @time "include file $file" Base.include((globalThis as any).JulGame.ScriptModule, file)
        ...
        for (const entity of (globalThis as any).MAIN.scene.entities) {
            ...
            for (const script of entity.scripts) {
                ...
                try {
                    ...
                    @time "script construct: $(script.name)" newScript = constructor()
