...
function canonical_transpile_project_jl()::String
    return raw"""
# Generated by JulGame scaffold — game-specific transpile hooks.

# Optional: `include(...)` basenames not inlined during transpile (merged with engine defaults).
# const SKIP_GAME_SCRIPT_INLINE = Set(["MyHeavyModule.jl"])

# Optional transpile-time fixups (JulGame calls this from apply_game_script_fixups).
# function apply_project_script_fixups(content::AbstractString, name::AbstractString, path_jl::AbstractString)::String
#     return String(content)
# end

# Post-process one transpiled script under ts/_generated/scripts/.
...
function postprocess_after_transpile(project_root::AbstractString)
    ...
    isdir(scripts_out) || return
    for f in readdir(scripts_out)
        endswith(f, ".ts") || continue
        f == "index.ts" && continue
        ts_path = joinpath(scripts_out, f)
        jl_path = joinpath(scripts_jl, "$(splitext(f)[1]).jl")
        original = read(ts_path, String)
        updated = postprocess_script_ts(
            original,
            ts_path,
            isfile(jl_path) ? jl_path : "",
        )
        if updated != original
            open(ts_path, "w") do io
                print(io, updated)
            end
            println("postprocess: $(relpath(ts_path, project_root))")
        end
    end
end
"""
...
function write_transpile_project_jl(project_root::AbstractString, force::Bool)
    dest = joinpath(abspath(project_root), "tools", "transpile_project.jl")
    if isfile(dest) && !force
        println("skip (exists): $dest")
        return false
    end
    mkpath(dirname(dest))
    open(dest, "w") do io
        print(io, canonical_transpile_project_jl())
    end
    println("wrote: $dest")
    return true
end
...
function main()
    ...
    write_transpile_project_jl(opts.project_root, opts.force)
    ...
    println("  npm run dev")
    println("  # → http://localhost:$(opts.port)/")
end
