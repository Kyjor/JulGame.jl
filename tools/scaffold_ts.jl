...
function main()
    ...
    convert_script = replace(relpath(joinpath(julgame, "tools", "convert.jl"), opts.project_root), '\\' => '/')
    julgame_ts_rel = replace(relpath(joinpath(julgame, "ts"), opts.project_root), '\\' => '/')
    julgame_ts_dir = replace(relpath(joinpath(julgame, "ts"), opts.project_root), '\\' => '/')
