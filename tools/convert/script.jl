...
function apply_game_script_fixups(data::AbstractString, name::AbstractString)::String
    ...
    s = replace(s, r"\( \(col: any\) => \$2\(self, col\)" => "(col: any) => handle_collisions(self, col)")
    s = replace(s, r"\( \(col: any\) => handle_bullet_collisions\(self, col\)" => "(col: any) => handle_bullet_collisions(self, col)")
    ...
    s = replace(s, r"\bMAIN\." => "(globalThis as any).MAIN.")
    s = replace(s, r"\bMAIN\b(?!\.)" => "(globalThis as any).MAIN")
...
function postprocess_game_script_ts(data::AbstractString, path_jl::AbstractString, path_ts::AbstractString)::String
    ...
    return insert_semicolon_before_line_starting_with_open_paren(s)
end
