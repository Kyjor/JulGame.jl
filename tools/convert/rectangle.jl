# Rectangle.jl → Rectangle.ts (filled rect render path for wasm).

function is_rectangle_source(path_jl::AbstractString)::Bool
    return endswith(replace(normpath(String(path_jl)), '\\' => '/'), "/src/engine/UI/Rectangle.jl")
end

function postprocess_rectangle_ts(_data::AbstractString)::String
    return """
