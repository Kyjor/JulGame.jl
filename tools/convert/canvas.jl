# Canvas.jl → Canvas.ts (container + visibility helpers from Julia CanvasModule / MainLoop UI pass).

function is_canvas_source(path_jl::AbstractString)::Bool
    return endswith(replace(normpath(String(path_jl)), '\\' => '/'), "/src/engine/UI/Canvas.jl")
end

function postprocess_canvas_ts(_data::AbstractString)::String
    return """
